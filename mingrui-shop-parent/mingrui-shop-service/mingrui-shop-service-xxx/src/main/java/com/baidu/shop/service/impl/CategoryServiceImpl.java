...
public class CategoryServiceImpl extends BaseApiService implements CategoryService {
    ...
    public Result<List<CategoryEntity>> getCategoryByIdList(String categoryIds) {
        ...
        List<Integer> categoryIdsArr = Arrays.asList(categoryIds.split(","))
                .stream().map(idStr -> Integer.parseInt(idStr))
                .collect(Collectors.toList());
        List<CategoryEntity> list = categoryMapper.selectByIdList(categoryIdsArr);
        return this.setResultSuccess(list);
    }
}
